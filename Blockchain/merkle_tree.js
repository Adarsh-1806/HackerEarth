...
function get_all_hashes(transactions){
    for (let i = 0; i < transactions.length; i++) {
        hashlist.push(gethash(JSON.stringify(transactions[i])));
    }
}
...
if(hashlist.length == 0){return null};
   ...
   for(let i=0;i<hashlist.length-1;i+=2){
        new_hashlist.push(gethash(hashlist[i]+hashlist[i+1]));
    }
    //for last single Element
    if((hashlist.length)%2 == 1){
        new_hashlist.push(gethash(hashlist[hashlist.length-1]+hashlist[hashlist.length-1]));
    }
